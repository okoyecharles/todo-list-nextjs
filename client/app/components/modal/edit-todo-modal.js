...
export default function EditTodoModal({ todo, open, setOpen, editTodo }) {
  ...
  const handleEdit = () => {
    ...
    const res = editTodo(todo.id, fields);
  ...
  useEffect(() => {
    if (!open) return;
    setFields({ name: todo.name, description: todo.description });
    setError(null);
    nameField.current.focus()
  }, [open]);

  return (
    <Modal header="Edit Todo" open={open} setOpen={setOpen}>
